...
export default function LoginComponent() {
    ...
    const emailRegex = /^[a-z]+[0-9]?[@]+[a-z]+[.]+[a-z]{3}$/
    ...
    const Submit = (e) => {
        ...
        if (emailRegex.test(loginObj.email)) {
            ...
            setborderE('1px solid red')
            seterrorEMsg('Invalid Email!')
        }
        if (passwordRegex.test(loginObj.password)) {
            ...
            setborderP('1px solid red')
            seterrorPMsg('Invalid Password!')
        }
        ...
        <>
            <div className='mainlogin'>
                ...
                <div className='detailslogin'>
                    <Input style={{ border: borderE }} onChange={TakeEmail} placeholder='Email' />
                    <i style={{ color: 'red', fontSize: '12px' }}>{errorEMsg}</i>
                    <Input style={{ border: borderP }} onChange={TakePassword} type='password' placeholder='Password' />
                    <p><i style={{ color: 'red', fontSize: '12px' }}>{errorPMsg}</i></p>
                    <Button className='emailbtnlogin'>Forgot password?</Button>
                </div>
                <div className='learnmorelogin'>
                    <p>Not your computer? Use Guest mode to sign in privately.<br /> <a href="#components-anchor-demo-static">Learn more</a></p>

                </div>
                <div className='nextlogin'>
                    <Button onClick={() => history.push('/signUp')} className='btnclogin' >Create account</Button>
                    <Button onClick={Submit} className='nextbtnclogin'>Next</Button>
                </div>
                <p style={{ color: 'red', fontSize: '12px' }}>{responseError}</p>
            </div>
        </>
    )
}
